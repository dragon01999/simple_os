...
void write_time_into(struct tm *time) 
{
    ...
    time->year =  *rtcdr / YEAR_SEC;
	time->mon  = (*rtcdr / MONTH_SEC) % 13;
	time->day  = (*rtcdr / DAY_SEC) % 30;
	time->hr   = (*rtcdr / HOUR_SEC) % 24;
	time->min  = (*rtcdr / MINUTE_SEC) % 60;
	time->sec  =  *rtcd % 60;
}
